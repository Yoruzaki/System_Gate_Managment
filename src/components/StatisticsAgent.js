...
import { FaUsers, FaCar, FaCalendarDay, FaCalendarAlt, FaSync, FaArrowLeft, FaArrowRight, FaBell } from "react-icons/fa";
...
const StatisticsAgent = () => {
  ...
  const sendNotification = () => {
    console.log("Sending notification:", { notificationName, notificationMessage }); // Debug
    fetch("http://localhost:5000/send-notification", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: notificationName, message: notificationMessage }),
    })
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        console.log("Notification sent:", data); // Debug
        alert(data.message);
        setIsNotificationModalOpen(false);
      })
      .catch((err) => {
        console.error("Error sending notification:", err); // Debug
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-end mb-6">
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-indigo-500 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-600 transition-colors duration-300"
        >
